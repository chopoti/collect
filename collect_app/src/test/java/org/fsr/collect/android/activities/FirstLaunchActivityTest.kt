...
class FirstLaunchActivityTest {
    ...
    fun `The QrCodeProjectCreatorDialog should be displayed after clicking on the 'Configure with QR code' button`() {
        ...
        scenario.onActivity {
            onView(withText(org.fsr.collect.strings.R.string.configure_with_qr_code)).perform(click())
            assertThat(
    ...
    fun `The ManualProjectCreatorDialog should be displayed after clicking on the 'Configure manually' button`() {
        val scenario = launcherRule.launch(FirstLaunchActivity::class.java)
        scenario.onActivity {
            onView(withText(org.fsr.collect.strings.R.string.configure_manually)).perform(click())
            assertThat(
            ...
            onView(
                withText(
                    ApplicationProvider.getApplicationContext<Collect>().getLocalizedString(
                        org.fsr.collect.strings.R.string.collect_app_name
                    ) + " vfake"
                )
        ...
        scenario.onActivity {
            ...
            onView(
                withText(
                    containsString(
                        ApplicationProvider.getApplicationContext<Collect>().getLocalizedString(
                            org.fsr.collect.strings.R.string.try_demo
                        )
                    )
                )
