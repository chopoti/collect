...
import org.fsr.collect.permissions.R
...
class ResetProgressDialogTest {
    ...
    fun `The dialog should display proper content`() {
        ...
        scenario.onFragment {
            // Title
            assertThat(it.title, `is`(context.getLocalizedString(org.fsr.collect.strings.R.string.please_wait)))

            // Message
            assertThat(it.message, `is`(context.getLocalizedString(org.fsr.collect.strings.R.string.reset_in_progress)))
        }
    }
}
