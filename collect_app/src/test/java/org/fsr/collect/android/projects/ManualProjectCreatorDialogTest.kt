...
import org.fsr.collect.android.support.Matchers.isPasswordHidden
...
class ManualProjectCreatorDialogTest {
    ...
    fun `Password should be protected`() {
        ...
        scenario.onFragment {
            onView(withHint(org.fsr.collect.strings.R.string.server_url)).inRoot(isDialog())
                .perform(replaceText("123456789"))
            onView(withHint(org.fsr.collect.strings.R.string.server_url)).inRoot(isDialog())
                .check(matches(not(isPasswordHidden())))

            onView(withHint(org.fsr.collect.strings.R.string.username)).inRoot(isDialog()).perform(replaceText("123456789"))
            onView(withHint(org.fsr.collect.strings.R.string.username)).inRoot(isDialog())
                .check(matches(not(isPasswordHidden())))

            onView(withHint(org.fsr.collect.strings.R.string.password)).inRoot(isDialog()).perform(replaceText("123456789"))
            onView(withHint(org.fsr.collect.strings.R.string.password)).inRoot(isDialog())
    ...
    fun `The 'Add' button should be disabled when url is blank`() {
        ...
        scenario.onFragment {
            ...
            onView(withText(org.fsr.collect.strings.R.string.add)).inRoot(isDialog()).perform(click())
            assertThat(it.isVisible, `is`(true))

            onView(withHint(org.fsr.collect.strings.R.string.server_url)).inRoot(isDialog()).perform(replaceText(" "))
            onView(withText(org.fsr.collect.strings.R.string.add)).inRoot(isDialog()).perform(click())
            assertThat(it.isVisible, `is`(true))
        }
    }
    ...
    fun `When URL has no protocol, a toast is displayed`() {
        ...
        scenario.onFragment {
            ...
            assertThat(toastText, `is`(it.getString(org.fsr.collect.strings.R.string.url_error)))
        }
    }
    ...
    fun `Server project creation should be triggered after clicking on the 'Add' button`() {
        ...
        scenario.onFragment {
            ...
            onView(withHint(org.fsr.collect.strings.R.string.username)).inRoot(isDialog()).perform(replaceText("adam"))
            onView(withHint(org.fsr.collect.strings.R.string.password)).inRoot(isDialog()).perform(replaceText("1234"))
