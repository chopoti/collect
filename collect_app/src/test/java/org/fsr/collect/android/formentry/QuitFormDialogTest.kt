...
import org.fsr.collect.settings.keys.ProtectedProjectKeys.KEY_SAVE_MID
import org.fsr.collect.shadows.ShadowAndroidXAlertDialog
...
class QuitFormDialogTest {
    ...
    fun whenSaveAsDraftIsEnabled_andLastSavedTimeIsNull_showsSaveExplanation() {
        ...
        assertThat(
            ...
            equalTo(activity.getString(org.fsr.collect.strings.R.string.save_explanation))
        )
    }
    ...
    fun whenSaveAsDraftIsEnabled_andLastSavedTimeIsNotNull_showsLastSavedTime() {
        ...
        assertThat(
            ...
            equalTo(
                SimpleDateFormat(
                    activity.getString(org.fsr.collect.strings.R.string.save_explanation_with_last_saved),
                    Locale.getDefault()
                ).format(456L)
            )
        )
    ...
    fun whenSaveAsDraftIsDisabled_andLastSavedTimeIsNull_showsWarningTitleAndMessage_andHidesButton() {
        ...
        assertThat(
            shadowDialog.title,
            equalTo(activity.getString(org.fsr.collect.strings.R.string.quit_form_continue_title))
        )
        assertThat(
            shadowDialog.getView().findViewById<TextView>(R.id.save_explanation).text,
            equalTo(activity.getString(org.fsr.collect.strings.R.string.discard_form_warning))
        )
        assertThat(
    ...
    fun whenSaveAsDraftIsDisabled_andLastSavedTimeIsNotNull_showsWarningTitleAndMessage_andHidesButton() {
        ...
        assertThat(
            ...
            equalTo(
                SimpleDateFormat(
                    activity.getString(org.fsr.collect.strings.R.string.discard_changes_warning),
                    Locale.getDefault()
                ).format(456L)
            )
        )
