...
class FormsDownloadResultPage : Page<FormsDownloadResultPage>() {
    ...
    override fun assertOnPage(): FormsDownloadResultPage {
        onView(withText(org.fsr.collect.strings.R.string.ok)).inRoot(RootMatchers.isDialog()).check(matches(isDisplayed()))
        return this
    }
    ...
    fun assertSuccess(): FormsDownloadResultPage {
        assertText(org.fsr.collect.strings.R.string.all_downloads_succeeded)
        return this
    }

    fun showDetails(): ErrorPage {
        onView(withText(getTranslatedString(org.fsr.collect.strings.R.string.show_details))).perform(click())
