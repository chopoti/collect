...
class SaveOrDiscardFormDialog<D : Page<D>> @JvmOverloads constructor(
    ...
    override fun assertOnPage(): SaveOrDiscardFormDialog<D> {
        if (saveAsDraftEnabled) {
            assertTextInDialog(org.fsr.collect.strings.R.string.quit_form_title)
        } else {
            assertTextInDialog(org.fsr.collect.strings.R.string.quit_form_continue_title)
        }
    ...
    fun clickSaveChanges(): D {
        return clickOnTextInDialog(org.fsr.collect.strings.R.string.save_as_draft, destination)
    }

    fun clickSaveChangesWithError(errorMsg: Int): D {
        clickOnTextInDialog(org.fsr.collect.strings.R.string.save_as_draft)
    ...
    fun clickDiscardForm(): D {
        return clickOnTextInDialog(org.fsr.collect.strings.R.string.do_not_save, destination)
    }
