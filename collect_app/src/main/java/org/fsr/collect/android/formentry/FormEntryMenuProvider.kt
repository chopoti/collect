...
import org.fsr.collect.android.formentry.questions.AnswersProvider
...
import org.fsr.collect.androidshared.ui.DialogFragmentUtils.showIfNotShowing
import org.fsr.collect.androidshared.ui.enableIconsVisibility
import org.fsr.collect.androidshared.ui.multiclicksafe.MultiClickGuard.allowClick
    ...
    override fun onPrepareMenu(menu: Menu) {
        ...
        if (formController != null &&
            ...
            menu.findItem(R.id.track_location).icon = if (isBackgroundLocationEnabled) {
                ContextCompat.getDrawable(activity, org.fsr.collect.icons.R.drawable.ic_baseline_location_on_24)
            } else {
                ContextCompat.getDrawable(activity, org.fsr.collect.icons.R.drawable.ic_baseline_location_off_24)
            }
            menu.findItem(R.id.track_location).title = if (isBackgroundLocationEnabled) {
                activity.getLocalizedString(org.fsr.collect.strings.R.string.track_location_on)
            } else {
                activity.getLocalizedString(org.fsr.collect.strings.R.string.track_location_off)
            }
        }
        ...
        menu.findItem(R.id.menu_record_audio).icon = if (isRecordingAudioEnabled) {
            ContextCompat.getDrawable(activity, org.fsr.collect.icons.R.drawable.ic_baseline_mic_24)
        } else {
            ContextCompat.getDrawable(activity, org.fsr.collect.icons.R.drawable.ic_baseline_mic_off_24)
        }
        menu.findItem(R.id.menu_record_audio).title = if (isRecordingAudioEnabled) {
            activity.getLocalizedString(org.fsr.collect.strings.R.string.record_audio_on)
        } else {
            activity.getLocalizedString(org.fsr.collect.strings.R.string.record_audio_off)
        }
    }
    ...
    override fun onMenuItemSelected(item: MenuItem): Boolean {
        ...
        return when (item.itemId) {
            ...
            R.id.menu_record_audio -> {
                val enabled = item.title == activity.getLocalizedString(org.fsr.collect.strings.R.string.record_audio_off)
                if (enabled) {
                    MaterialAlertDialogBuilder(activity)
                        .setMessage(org.fsr.collect.strings.R.string.background_audio_recording_enabled_explanation)
                        .setCancelable(false)
                    ...
                    MaterialAlertDialogBuilder(activity)
                        .setMessage(org.fsr.collect.strings.R.string.stop_recording_confirmation)
                        .setPositiveButton(org.fsr.collect.strings.R.string.disable_recording) { _: DialogInterface?, _: Int -> backgroundAudioViewModel.setBackgroundRecordingEnabled(false) }
