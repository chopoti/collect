    ...
    override fun onCreateMenu(menu: Menu, inflater: MenuInflater) {
        ...
        menu.findItem(R.id.menu_filter).apply {
            ...
            (actionView as SearchView).apply {
                queryHint = activity.resources.getString(org.fsr.collect.strings.R.string.search)
            ...
            R.id.menu_refresh -> {
                if (networkStateProvider?.isDeviceOnline == true) {
                    viewModel.syncWithServer().observe(activity) { success: Boolean ->
                        if (success) {
                            ToastUtils.showShortToast(activity, org.fsr.collect.strings.R.string.form_update_succeeded)
                        }
                    }
                } else {
                    ToastUtils.showShortToast(activity, org.fsr.collect.strings.R.string.no_connection)
                }
                true
            }
    ...
    private fun getForListSortingOption(it: BlankFormListViewModel.SortOrder) =
        when (it) {
            ...
            BlankFormListViewModel.SortOrder.LAST_SAVED -> FormListSortingOption(
                ...
                org.fsr.collect.strings.R.string.sort_by_last_saved
            )
        }
}
