...
import org.fsr.collect.errors.ErrorActivity
...
class FormsDownloadResultDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val builder = MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(getString(org.fsr.collect.strings.R.string.ok)) { _, _ ->
                listener?.onCloseDownloadingResult()
            }

        if (!FormsDownloadResultInterpreter.allFormsDownloadedSuccessfully(result)) {
            builder.setNegativeButton(getString(org.fsr.collect.strings.R.string.show_details)) { _, _ ->
    ...
    private fun getMessage(): String {
        return if (FormsDownloadResultInterpreter.allFormsDownloadedSuccessfully(result)) {
            getString(org.fsr.collect.strings.R.string.all_downloads_succeeded)
        } else {
            getString(org.fsr.collect.strings.R.string.some_downloads_failed, FormsDownloadResultInterpreter.getNumberOfFailures(result).toString(), result.size.toString())
        }
    }
