    ...
    fun synchronizeWithDisk(formsRepository: FormsRepository, formsDir: String?): String {
        ...
        return try {
            ...
            if (formDir.exists() && formDir.isDirectory) {
                // Get all the files in the /fsr/foms directory
            ...
            if (errors.length != 0) {
                ...
                Timber.d(
                    Collect.getInstance()
                        .getLocalizedString(org.fsr.collect.strings.R.string.finished_disk_scan)
                )
            }
            statusMessage
        } finally {
    ...
    private fun parseForm(formDefFile: File?): Form {
        ...
        if (title != null) {
            ...
            throw IllegalArgumentException(
                Collect.getInstance()
                    .getLocalizedString(
                        org.fsr.collect.strings.R.string.xform_parse_error,
                        formDefFile!!.name,
                        "title"
                    )
            )
