...
import org.fsr.collect.androidshared.ui.addOnClickListener
import org.fsr.collect.async.Scheduler
import org.fsr.collect.lists.selects.MultiSelectViewModel
import org.fsr.collect.lists.selects.SelectItem
import org.fsr.collect.lists.selects.SingleSelectViewModel
import org.fsr.collect.maps.databinding.OfflineMapLayersPickerBinding
        ...
        MaterialAlertDialogBuilder(requireActivity())
            .setMessage(
                requireActivity().getLocalizedString(
                    org.fsr.collect.strings.R.string.delete_layer_confirmation_message,
                    layerItem.name
                )
            )
            .setPositiveButton(org.fsr.collect.strings.R.string.delete_layer) { _, _ ->
    ...
    private fun updateAdapter(
        ...
        val newData = mutableListOf(
            CheckableReferenceLayer(
                null,
                null,
                requireContext().getLocalizedString(org.fsr.collect.strings.R.string.none),
                checkedLayerId == null,
                false
            )
        )
