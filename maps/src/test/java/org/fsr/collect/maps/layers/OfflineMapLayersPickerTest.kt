...
import org.fsr.collect.androidtest.DrawableMatcher.withImageDrawable
import org.fsr.collect.fragmentstest.FragmentScenarioLauncherRule
import org.fsr.collect.maps.R
...
class OfflineMapLayersPickerTest {
    ...
    private fun assertLayerCollapsed(position: Int) {
        onView(withRecyclerView(R.id.layers).atPositionOnView(position, R.id.arrow)).check(
            matches(
                withImageDrawable(org.fsr.collect.icons.R.drawable.ic_baseline_expand_24)
            )
        )
    ...
    private fun assertLayerExpanded(position: Int) {
        onView(withRecyclerView(R.id.layers).atPositionOnView(position, R.id.arrow)).check(
            matches(
                withImageDrawable(org.fsr.collect.icons.R.drawable.ic_baseline_collapse_24)
            )
        )
        onView(withRecyclerView(R.id.layers).atPositionOnView(position, R.id.path)).check(
            matches(
