...
package org.fsr.collect.geo.geopoly;

import static org.fsr.collect.androidshared.system.ContextUtils.getThemeAttributeValue;
import static org.fsr.collect.geo.Constants.EXTRA_READ_ONLY;
import static org.fsr.collect.geo.GeoActivityUtils.requireLocationPermissions;
...
import org.fsr.collect.geo.Constants;
...
import org.fsr.collect.maps.LineDescription;
import org.fsr.collect.maps.MapConsts;
...
import org.fsr.collect.maps.layers.OfflineMapLayersPicker;
...
public class GeoPolyActivity extends LocalizedActivity implements GeoPolySettingsDialogFragment.SettingsDialogCallback {
    ...
    @Override public void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(getString(outputMode == OutputMode.GEOTRACE ?
            org.fsr.collect.strings.R.string.geotrace_title : org.fsr.collect.strings.R.string.geoshape_title));
    ...
    private void saveAsPolyline() {
        if (map.getPolyLinePoints(featureId).size() > 1) {
            ...
            ToastUtils.showShortToastInMiddle(this, getString(org.fsr.collect.strings.R.string.polyline_validator));
        }
    }
    ...
    private void saveAsPolygon() {
        if (map.getPolyLinePoints(featureId).size() > 2) {
            ...
            ToastUtils.showShortToastInMiddle(this, getString(org.fsr.collect.strings.R.string.polygon_validator));
        }
    }
    ...
    private void updateUi() {
        ...
        locationStatus.setText(
            location == null ? getString(org.fsr.collect.strings.R.string.location_status_searching)
                : !usingThreshold ? getString(org.fsr.collect.strings.R.string.location_status_accuracy, location.accuracy)
                : acceptable ? getString(org.fsr.collect.strings.R.string.location_status_acceptable, location.accuracy)
                : getString(org.fsr.collect.strings.R.string.location_status_unacceptable, location.accuracy)
        );
        ...
        collectionStatus.setText(
            !inputActive ? getString(org.fsr.collect.strings.R.string.collection_status_paused, numPoints)
                : !recordingEnabled ? getString(org.fsr.collect.strings.R.string.collection_status_placement, numPoints)
                : !recordingAutomatic ? getString(org.fsr.collect.strings.R.string.collection_status_manual, numPoints)
                : !usingThreshold ? (
                    minutes > 0 ?
                        getString(org.fsr.collect.strings.R.string.collection_status_auto_minutes, numPoints, minutes) :
                        getString(org.fsr.collect.strings.R.string.collection_status_auto_seconds, numPoints, seconds)
                )
                : (
                    minutes > 0 ?
                        getString(org.fsr.collect.strings.R.string.collection_status_auto_minutes_accuracy, numPoints, minutes, meters) :
                        getString(org.fsr.collect.strings.R.string.collection_status_auto_seconds_accuracy, numPoints, seconds, meters)
                )
        );
    }
    ...
    private void showClearDialog() {
        if (!map.getPolyLinePoints(featureId).isEmpty()) {
            new MaterialAlertDialogBuilder(this)
                .setMessage(org.fsr.collect.strings.R.string.geo_clear_warning)
                .setPositiveButton(org.fsr.collect.strings.R.string.clear, (dialog, id) -> clear())
                .setNegativeButton(org.fsr.collect.strings.R.string.cancel, null)
                .show();
        }
    }
    ...
    private void showBackDialog() {
        new MaterialAlertDialogBuilder(this)
            .setMessage(getString(org.fsr.collect.strings.R.string.geo_exit_warning))
            .setPositiveButton(org.fsr.collect.strings.R.string.discard, (dialog, id) -> finish())
            .setNegativeButton(org.fsr.collect.strings.R.string.cancel, null)
            .show();
