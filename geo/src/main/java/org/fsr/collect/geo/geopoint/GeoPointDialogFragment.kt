...
import org.fsr.collect.geo.databinding.GeopointDialogBinding
...
class GeoPointDialogFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        binding.threshold.text =
            getString(org.fsr.collect.strings.R.string.point_will_be_saved, formatAccuracy(context, accuracyThreshold))

        viewModel.timeElapsed.observe(this) {
            binding.time.text =
                getString(org.fsr.collect.strings.R.string.time_elapsed, DateUtils.formatElapsedTime(it / 1000))
        }

        viewModel.satellites.observe(this) {
            binding.satellites.text = getString(org.fsr.collect.strings.R.string.satellites, it.toString())
        }

        val dialog = MaterialAlertDialogBuilder(requireContext())
            .setView(binding.root)
            .setPositiveButton(org.fsr.collect.strings.R.string.save) { _, _ -> viewModel.forceLocation() }
            .setNegativeButton(org.fsr.collect.strings.R.string.cancel) { _, _ -> listener?.onCancel() }
            .create()
